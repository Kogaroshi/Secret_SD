...
if(sys.platform == "win32"):
    ...
    for device in iter(monitor.poll, None):
        if device.action == 'add':
            ...
            if CryptoSD.check_keys("","cleSD",device.sysname):
                cardPath = device.sysname
                print("Right SD Card at : "+cardPath)
                #unlockAllLinux()
        if device.action == 'remove':
            print('{} disconnected'.format(device))
            print(device.sys_name)
            if device.sysname == cardPath:
                print("SD Card removed at : "+cardPath)
                #lockAllLinux()
                cardPath = None
